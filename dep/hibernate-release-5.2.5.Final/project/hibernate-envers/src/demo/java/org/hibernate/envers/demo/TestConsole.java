...
public class TestConsole {
    ...
    private void printPersons(StringBuilder sb) {
        List<Person> persons = entityManager.createQuery(
                "select p from Entities.Person p order by p.id").getResultList();
    ...
    private Person readNewPerson(PrintStream out, Scanner scanner) {
        ...
        out.print("Entities.Person name (NULL for null): ");
        p.setName(convertString(scanner.nextLine(), ""));

        out.print("Entities.Person surname (NULL for null): ");
        p.setSurname(convertString(scanner.nextLine(), ""));

        out.print("Entities.Person address id (NULL for null): ");
    ...
    private void readModifyPerson(PrintStream out, Scanner scanner, int personId) {
        ...
        if (current == null) {
            out.println("Entities.Person with id " + personId + " does not exist.");
            return;
        }

        out.print("Entities.Person name (NULL for null, enter for no change, current - " + current.getName() + "): ");
        current.setName(convertString(scanner.nextLine(), current.getName()));

        out.print("Entities.Person surname (NULL for null, enter for no change, current - " + current.getSurname() + "): ");
        current.setSurname(convertString(scanner.nextLine(), current.getSurname()));

        out.print("Entities.Person address id (NULL for null, enter for no change, current - " +
    ...
    private boolean hasData() {
        return (((Long) entityManager.createQuery("select count(a) from Address a").getSingleResult()) +
                ((Long) entityManager.createQuery("select count(p) from Entities.Person p").getSingleResult())) > 0;
    }
