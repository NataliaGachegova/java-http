...
public class ElementCollectionSortingTest extends BaseCoreFunctionalTestCase {
	...
	public void testSortingElementCollectionSyntax() {
		...
		session.createQuery( "from Entities.Person p join fetch p.nickNamesAscendingNaturalSort" ).list();
		session.createQuery( "from Entities.Person p join fetch p.nickNamesDescendingNaturalSort" ).list();

		session.createQuery( "from Entities.Person p join fetch p.addressesAscendingNaturalSort" ).list();
		session.createQuery( "from Entities.Person p join fetch p.addressesDescendingNaturalSort" ).list();
		session.createQuery( "from Entities.Person p join fetch p.addressesCityAscendingSort" ).list();
		session.createQuery( "from Entities.Person p join fetch p.addressesCityDescendingSort" ).list();
	...
	public void testSortingEmbeddableCollectionOfPrimitives() {
		...
		final List<Person> result = session.createQuery(
				"select distinct p from Entities.Person p join fetch p.nickNamesAscendingNaturalSort join fetch p.nickNamesDescendingNaturalSort order by p.name"
		).list();
