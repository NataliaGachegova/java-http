...
public class UnionSubclassTest extends BaseCoreFunctionalTestCase {
	protected String[] getMappings() {
		return new String[] { "unionsubclass2/Entities.Person.hbm.xml" };
	}
	...
	public void testUnionSubclass() {
		...
		assertEquals( s.createQuery("from Entities.Person").list().size(), 3 );
		assertEquals( s.createQuery("from Entities.Person p where p.class = Customer").list().size(), 1 );
		assertEquals( s.createQuery("from Entities.Person p where p.class = Entities.Person").list().size(), 1 );
		assertEquals( s.createQuery("from Entities.Person p where type(p) in :who").setParameter("who", Customer.class).list().size(), 1 );
		assertEquals( s.createQuery("from Entities.Person p where type(p) in :who").setParameterList("who", new Class[] {Customer.class, Person.class}).list().size(), 2 );
		s.clear();
		...
		joe = (Customer) s.get( Customer.class, Long.valueOf( joe.getId() ) );
 		...
 		mark.setZip("30306");
		assertEquals( s.createQuery("from Entities.Person p where p.address.zip = '30306'").list().size(), 1 );
		...
		s.delete(joe);
		s.delete(yomomma);
		assertTrue( s.createQuery("from Entities.Person").list().isEmpty() );
		t.commit();
		s.close();
	}
	...
	public void testQuerySubclassAttribute() {
		...
		List result = s.createQuery("from Entities.Person where salary > 100").list();
		assertEquals( result.size(), 1 );
		assertSame( result.get(0), q );
		
		result = s.createQuery("from Entities.Person where salary > 100 or name like 'E%'").list();
	...
	public void testCustomColumnReadAndWrite() {
		...
		// Test projection
		Double heightViaHql = (Double)s.createQuery("select p.heightInches from Entities.Person p where p.name = 'Emmanuel'").uniqueResult();
		...
		p = (Person)s.createQuery("from Entities.Person p where p.heightInches between ? and ?")
