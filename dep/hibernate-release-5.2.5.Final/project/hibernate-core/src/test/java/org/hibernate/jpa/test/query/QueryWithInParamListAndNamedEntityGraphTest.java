...
public class QueryWithInParamListAndNamedEntityGraphTest extends BaseEntityManagerFunctionalTestCase {
	...
	public void testInClause() {
		...
		ids.add( 1L );
		ids.add( 2L );
		TypedQuery<Person> query = em.createQuery( "select p from Entities.Person p where p.id  in :ids", Person.class );
	...
	public void testEntityGraph() {
		...
		EntityManager em = getOrCreateEntityManager();
		em.getTransaction().begin();
		TypedQuery<Person> query = em.createQuery( "select p from Entities.Person p", Person.class );
