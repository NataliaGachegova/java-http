...
public class JoinTest extends BaseCoreFunctionalTestCase {
	...
	public String[] getMappings() {
		return new String[] { "join/Entities.Person.hbm.xml" };
	}
	...
	public void testSequentialSelects() {
		...
		assertEquals( s.createQuery("from Entities.Person p where p.class is null").list().size(), 1 );
		...
		joe = (Customer) s.get( Customer.class, new Long( joe.getId() ) );
 		...
 		mark.setZip("30306");
		assertEquals( s.createQuery("from Entities.Person p where p.zip = '30306'").list().size(), 1 );
		s.delete(mark);
		s.delete(joe);
		s.delete(yomomma);
	...
	public void testSequentialSelectsOptionalData() throws Exception {
		...
		assertEquals( s.createQuery("from Entities.Person").list().size(), 1 );
		assertEquals( s.createQuery("from Entities.Person p where p.class is null").list().size(), 0 );
		assertEquals( s.createQuery("from Entities.Person p where p.class = User").list().size(), 1 );
