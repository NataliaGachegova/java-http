...
public class UnionSubclassFilterTest extends BaseCoreFunctionalTestCase {
	...
	public void testFiltersWithUnionSubclass() {
		...
		results = s.createQuery( "from Entities.Person" ).list();
		...
		results = new ArrayList( new HashSet( s.createQuery( "from Entities.Person as p left join fetch p.minions" ).list() ) );
		assertEquals( "Incorrect qry result count", 4, results.size() );
		...
		for ( Object entity : s.createQuery( "from Entities.Person" ).list() ) {
			s.delete( entity );
		}
		t.commit();
		s.close();
