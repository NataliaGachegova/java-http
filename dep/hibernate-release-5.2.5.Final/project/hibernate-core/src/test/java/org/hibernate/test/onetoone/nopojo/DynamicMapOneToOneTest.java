...
public class DynamicMapOneToOneTest extends BaseCoreFunctionalTestCase {
	...
	public String[] getMappings() {
		return new String[] { "onetoone/nopojo/Entities.Person.hbm.xml" };
	}
	...
	public void testOneToOneOnSubclass() {
		...
		s.beginTransaction();
		s.persist( "Entities.Person", person );
		...
		person = ( Map ) s.createQuery( "from Entities.Person p join fetch p.address" ).uniqueResult();
		assertNotNull( "could not locate person", person );
		assertNotNull( "could not locate persons address", person.get( "address" ) );
		s.clear();

		Object[] tuple = ( Object[] ) s.createQuery( "select p.name, p from Entities.Person p join fetch p.address" ).uniqueResult();
		assertEquals( tuple.length, 2 );
		person = ( Map ) tuple[1];
		assertNotNull( "could not locate person", person );
		assertNotNull( "could not locate persons address", person.get( "address" ) );

		s.delete( "Entities.Person", person );
