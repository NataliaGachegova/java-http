...
public class SimpleInheritanceTest extends BaseCoreFunctionalTestCase {
	...
	public void testDiscriminatorSubclass() {
		...
		assertEquals( s.createQuery("from org.hibernate.test.discriminator.Entities.Person").list().size(), 3 );
		assertEquals( s.createQuery("from org.hibernate.test.discriminator.Entities.Person p where p.class = org.hibernate.test.discriminator.Entities.Person").list().size(), 1 );
		assertEquals( s.createQuery("from org.hibernate.test.discriminator.Entities.Person p where p.class = org.hibernate.test.discriminator.Customer").list().size(), 1 );
		assertEquals( s.createQuery("from org.hibernate.test.discriminator.Entities.Person p where type(p) = :who").setParameter("who", Person.class).list().size(), 1 );
		assertEquals( s.createQuery("from org.hibernate.test.discriminator.Entities.Person p where type(p) in :who").setParameterList("who", new Class[] {Customer.class, Person.class}).list().size(), 2 );
		s.clear();
		...
		s.delete(joe);
		s.delete(yomomma);
		assertTrue( s.createQuery("from org.hibernate.test.discriminator.Entities.Person").list().isEmpty() );
		t.commit();
		s.close();
	}
	...
	public void testQuerySubclassAttribute() {
		...
		List result = s.createQuery("from org.hibernate.test.discriminator.Entities.Person where salary > 100").list();
		assertEquals( result.size(), 1 );
		assertSame( result.get(0), q );

		result = s.createQuery("from org.hibernate.test.discriminator.Entities.Person where salary > 100 or name like 'E%'").list();
		...
		//TODO: make this work:
		/*result = s.createQuery("select salary from Entities.Person where salary > 100").list();
		assertEquals( result.size(), 1 );
	...
	public void testLoadSuperclassProxyPolymorphicAccess() {
		...
		Person pQuery = ( Person ) s.createQuery( "from org.hibernate.test.discriminator.Entities.Person where id = :id" )
				.setLong( "id", e.getId() )
				.uniqueResult();
	...
	public void testLoadSuperclassProxyEvictPolymorphicAccess() {
		...
		Employee pQuery = ( Employee ) s.createQuery( "from org.hibernate.test.discriminator.Entities.Person where id = :id" )
				.setLong( "id", e.getId() )
				.uniqueResult();
