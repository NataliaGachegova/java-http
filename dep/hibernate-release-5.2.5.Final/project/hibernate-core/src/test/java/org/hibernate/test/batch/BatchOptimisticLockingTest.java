...
public class BatchOptimisticLockingTest extends
	...
	public void testBatchAndOptimisticLocking() {
		...
		try {
			doInHibernate( this::sessionFactory, session -> {
				List<Person> persons = session.createQuery( "select p from Entities.Person p").getResultList();

				for ( int i = 0; i < persons.size(); i++ ) {
					Person person = persons.get( i );
					person.name += " Entities.Person";
						...
						try {
							executorService.submit( () -> {
								doInHibernate( this::sessionFactory, _session -> {
									...
									_person.name += " Entities.Person is the new Boss!";
								} );
							} ).get();
						}
