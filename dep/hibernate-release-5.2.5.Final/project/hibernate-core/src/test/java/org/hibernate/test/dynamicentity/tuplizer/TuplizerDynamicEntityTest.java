...
public class TuplizerDynamicEntityTest extends BaseCoreFunctionalTestCase {
	...
	public void testIt() {
		...
		assertNotNull( "son:Entities.Person id not assigned", son.getId() );
		assertNotNull( "wife:Entities.Person id not assigned", wife.getId() );
		...
		session.clear();
		count = session.createQuery( "from Entities.Person" ).list().size();
