...
public class HqlParserTest {
	...
	@Test public void testDocoExamples97() throws Exception {
		...
		parse( "from bank.Entities.Person person\n"
...
//        parse("from Order order where minelement(order.items) > 10000");
		...
		parse( "select p from eg.NameList list, eg.Entities.Person p\n"
...
//        parse("select person from Entities.Person person, Calendar calendar\n"
...
//        + "where order.items[ maxindex(order.items) ] = item and order.id = 11");
	...
	@Test public void testUnitTestHql() throws Exception {
		...
		parse( "select new ChetNeChet.Result(foo.string, foo.long, foo.integer) from foo in class org.hibernate.test.Foo" );
		parse( "select new ChetNeChet.Result( baz.name, foo.long, count(elements(baz.fooArray)) ) from org.hibernate.test.Baz baz join baz.fooArray foo group by baz.name, foo.long" );
		parse( "select new ChetNeChet.Result( baz.name, max(foo.long), count(foo) ) from org.hibernate.test.Baz baz join baz.fooArray foo group by baz.name" );
...
//        parse("from sm in class org.hibernate.test.SubMulti where exists sm.children.elements");
    ...
    @Test public void testHHH1780() throws Exception {
        ...
        AST ast = doParse(
                "from Entities.Person p where not ( p.name is null and exists(select a.id from Address a where a.id=p.id))",
                false
        );
        ...
        ast = doParse(
                "from Entities.Person p where not ( p.name is null or exists(select a.id from Address a where a.id=p.id))",
                false
        );
