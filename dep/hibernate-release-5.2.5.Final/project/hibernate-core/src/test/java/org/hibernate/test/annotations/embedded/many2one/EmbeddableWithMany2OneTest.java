...
public class EmbeddableWithMany2OneTest extends BaseCoreFunctionalTestCase {
	...
	public void testJoinAcrossEmbedded() {
		...
		session.createQuery( "from Entities.Person p join p.address as a join a.country as c where c.name = 'US'" )
				.list();
		session.createQuery( "from Entities.Person p join p.address as a join a.country as c where c.id = 'US'" )
				.list();
	...
	public void testBasicOps() {
		...
		session = openSession();
		session.beginTransaction();
		session.createQuery( "from Entities.Person p where p.address.country.iso2 = 'US'" )
				.list();
		// same query!
		session.createQuery( "from Entities.Person p where p.address.country.id = 'US'" )
				.list();
