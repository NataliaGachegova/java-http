...
public class CallbackAndDirtyTest extends BaseEntityManagerFunctionalTestCase {
	...
	public void testDirtyButNotDirty() throws Exception {
		...
		manager.getTransaction().begin();
		assertEquals(
				manager.createQuery( "select p.address, p.name from Entities.Person p order by p.name" ).getResultList().size(),
				3
		);
		assertEquals( manager.createQuery( "select p from Entities.Person p where p.class = Customer" ).getResultList().size(), 1 );
		manager.getTransaction().commit();
		...
		mark.setZip( "30306" );
		assertEquals( 1, manager.createQuery( "select p from Entities.Person p where p.zip = '30306'" ).getResultList().size() );
		manager.remove( mark );
		manager.remove( joe );
		manager.remove( yomomma );
		assertTrue( manager.createQuery( "select p from Entities.Person p" ).getResultList().isEmpty() );
		manager.getTransaction().commit();
		manager.close();
	}
