...
public class CastNullSelectExpressionTest extends BaseCoreFunctionalTestCase {
	...
	public void testSelectNewCastNull() {
		...
		Person result = (Person) s.createQuery(
				"select new CastNullSelectExpressionTest$Entities.Person( id, firstName, cast( null as string ), lastName ) from CastNullSelectExpressionTest$Entities.Person where lastName='Munster'"
		).uniqueResult();
