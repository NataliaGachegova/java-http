...
public class IdentifierPropertyReferencesTest extends BaseCoreFunctionalTestCase {
	...
	public void testHqlIdPropertyReferences() {
		...
		long count = extractCount( s, "select count(*) from Entities.Person p where p.id = 123" );
		assertEquals( "Entities.Person by id prop (non-identifier)", 1, count );
		count = extractCount( s, "select count(*) from Entities.Person p where p.pk = 1" );
		assertEquals( "Entities.Person by pk prop (identifier)", 1, count );
