...
public class CompositePropertyRefTest extends BaseCoreFunctionalTestCase {
	public String[] getMappings() {
		return new String[] { "cuk/Entities.Person.hbm.xml" };
	}
	...
	public void testOneToOnePropertyRef() {
		...
		List l = s.createQuery("from Entities.Person").list(); //pull address references for cache
		assertEquals( l.size(), 2 );
		assertTrue( l.contains(p) && l.contains(p2) );
		s.clear();
		
		l = s.createQuery("from Entities.Person p order by p.name").list(); //get address references by sequential selects
		assertEquals( l.size(), 2 );
		assertNull( ( (Person) l.get(0) ).getAddress() );
		assertNotNull( ( (Person) l.get(1) ).getAddress() );
		s.clear();
		
		l = s.createQuery("from Entities.Person p left join fetch p.address a order by a.country").list(); //get em by outer join
		assertEquals( l.size(), 2 );
		...
		l = s.createQuery("from Entities.Person p left join p.accounts").list();
		for ( int i=0; i<2; i++ ) {
		...
		l = s.createQuery("from Entities.Person p left join fetch p.accounts a order by p.name").list();
		...
		l = s.createQuery("from Entities.Person p left join fetch p.address").list();
		
		s.clear();
		s.createQuery( "delete Address" ).executeUpdate();
		s.createQuery( "delete Account" ).executeUpdate();
		s.createQuery( "delete Entities.Person" ).executeUpdate();
		t.commit();
		s.close();
	}
