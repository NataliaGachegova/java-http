...
public class BatchTest extends BaseEntityManagerFunctionalTestCase {
	...
	public void testBulk() {
		...
		doInJPA( this::entityManagerFactory, entityManager -> {
			...
			int updatedEntities = session.createQuery(
				"update versioned Entities.Person " +
				"set name = :newName " +
				"where name = :oldName" )
			.setParameter( "oldName", oldName )
			.setParameter( "newName", newName )
	...
	private void withoutBatch() {
		...
		try {
			...
			for ( int i = 0; i < 100_000; i++ ) {
				Person Person = new Person( String.format( "Entities.Person %d", i ) );
	...
	private void withScroll() {
		...
		try {
			...
			scrollableResults = session
				.createQuery( "select p from Entities.Person p" )
