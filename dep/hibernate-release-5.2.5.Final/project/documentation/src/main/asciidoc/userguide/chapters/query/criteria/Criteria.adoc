...
The example uses `createQuery()` passing in the `Entities.Person` class reference as the results of the query will be `Entities.Person` objects.
...
In this example, the query is typed as `java.lang.String` because that is the anticipated type of the results (the type of the `Entities.Person#nickName` attribute is `java.lang.String`).
Because a query might contain multiple references to the `Entities.Person` entity, attribute references always need to be qualified.
...
Going back to the example query there, rather than returning an array of _[Entities.Person#id, Entities.Person#nickName]_, instead declare a class that holds these values and use that as a return object.
...
Here is an example defining a Cartesian Product between `Entities.Person` and `Partner` entities:
