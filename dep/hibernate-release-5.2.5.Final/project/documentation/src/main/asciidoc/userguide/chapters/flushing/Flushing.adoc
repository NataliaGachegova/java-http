...
The reason why the `Advertisement` entity query didn't trigger a flush is because there's no overlapping between the `Advertisement` and the `Entities.Person` tables:
...
When querying for a `Entities.Person` entity, the flush is triggered prior to executing the entity query.
