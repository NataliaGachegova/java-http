...
For example, if you cache results of a query against `Entities.Person`,
Hibernate will need to keep track of when those results should be invalidated because changes have been committed against any `Entities.Person` entity.
