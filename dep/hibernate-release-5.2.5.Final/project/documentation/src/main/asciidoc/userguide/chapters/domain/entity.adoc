...
Every time we load a specific `Entities.Person` from the database we would naturally get a unique instance.
...
So if we ask a Hibernate `Session` to load that specific Entities.Person multiple times we will actually get back the same __instance__:
...
Specifically the outcome in this last example will depend on whether the `Entities.Person` class implemented equals/hashCode, and, if so, how.
...
It turns out that this still breaks when adding transient instance of `Entities.Person` to a set as we saw in the last example:
...
When you need to modify the `Entities.Person` entity above:
