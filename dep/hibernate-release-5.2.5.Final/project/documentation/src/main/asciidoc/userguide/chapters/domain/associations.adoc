...
When persisting the `Entities.Person` entity, the cascade will propagate the persist operation to the underlying `Phone` children as well.
...
[source,java]
----
Entities.Person person1 = entityManager.find(Entities.Person.class, personId);
...
Both the `Entities.Person` and the `Address` have a` mappedBy` `@OneToMany` side, while the `PersonAddress` owns the `person` and the `address` `@ManyToOne` associations.
