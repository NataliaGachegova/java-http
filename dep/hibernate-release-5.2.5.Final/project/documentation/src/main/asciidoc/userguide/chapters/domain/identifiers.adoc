...
In the example above, the `PersonDetails` entity uses the `id` column for both the entity identifier and for the many-to-one association to the `Entities.Person` entity.
The value of the `PersonDetails` entity identifier is "derived" from the identifier of its parent `Entities.Person` entity.
