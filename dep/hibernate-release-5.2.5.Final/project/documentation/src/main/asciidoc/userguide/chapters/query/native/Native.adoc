...
Assuming that `Entities.Person` is mapped as a class with the columns `id`, `name`, `nickName`, `address`, `createdOn` and `version`,
the following query will also return a `List` where each element is a `Entities.Person` entity.
...
It is possible to eagerly join the `Phone` and the `Entities.Person` entities to avoid the possible extra roundtrip for initializing the `many-to-one` association.
...
The query was intended to return all `Entities.Person` and `Partner` instances with the same name.
...
[source, JAVA, indent=0]
----
include::{modeldir}/Entities.Person.java[tags=sql-scalar-NamedNativeQuery-example]
----
====
...
[source, JAVA, indent=0]
----
include::{modeldir}/Entities.Person.java[tags=sql-multiple-scalar-values-NamedNativeQuery-example]
----
====
...
[source, JAVA, indent=0]
----
include::{modeldir}/Entities.Person.java[tags=sql-multiple-scalar-values-dto-NamedNativeQuery-example]
----
====
...
[source, JAVA, indent=0]
----
include::{modeldir}/Entities.Person.java[tags=sql-entity-NamedNativeQuery-example]
----
====
...
[source, JAVA, indent=0]
----
include::{modeldir}/Entities.Person.java[tags=sql-entity-associations-NamedNativeQuery-example]
----
====
...
@NamedNativeQuery(
    ...
    resultClass = Entities.Person.class
)
----
...
The `Entities.Person` entity benefits from custom INSERT, UPDATE, and DELETE statements which update the `valid` column accordingly.
The custom `@Loader` is used to retrieve only `Entities.Person` rows that are valid.
