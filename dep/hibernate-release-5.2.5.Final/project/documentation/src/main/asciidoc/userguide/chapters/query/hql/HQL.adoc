...
----
include::{modeldir}/Entities.Person.java[tags=hql-examples-domain-model-example]
...
[source, JAVA, indent=0]
----
include::{modeldir}/Entities.Person.java[tags=jpql-api-named-query-example, indent=0]
...
When fetching a single result, like a `Entities.Person` entity, instead of a `Stream<Object[]>`, Hibernate is going to figure out the actual type, so the result is a `Stream<Entities.Person>`.
...
We see that the query is defining a root entity reference to the `org.hibernate.userguide.model.Entities.Person` object model type.
Additionally, it declares an alias of `p` to that `org.hibernate.userguide.model.Entities.Person` reference, which is the identification variable.

Usually, the root entity reference represents just the `entity name` rather than the entity class FQN (fully-qualified name).
By default, the entity name is the unqualified entity class name, here `Entities.Person`
...
As an example, given an entity named `Entities.Person` with a collection-valued association named `phones`, the `JOIN FETCH` will also load the child collection in the same SQL query:
...
In this case, `DISTINCT` is used because there can be multiple `Books` entities associated to a given `Entities.Person`.
...
In the example, the identification variable `ph` actually refers to the object model type `Phone`, which is the type of the elements of the `Entities.Person#phones` association.
